...
<div class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">EDIT <span class="text-emerald-600 uppercase">STAFF</span></h1>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1 underline decoration-rose-500/30 underline-offset-8">Pembaruan Data Akun Personel</p>
        </div>
        <a href="{{ route('admin.staffs.index') }}" class="flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-black text-[10px] uppercase tracking-widest transition-colors group">
    ...
    <!-- Form Card -->
    <div class="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.02)] relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent"></div>

        <form action="{{ route('admin.staffs.update', $staff) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2 space-y-2">
                    <label for="name" class="block text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Nama Lengkap Staff</label>
                ...
                <div class="space-y-2">
                    <label for="nip" class="block text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Nomor Induk Pegawai (NIP)</label>
                    <input 
                        type="text" 
                        name="nip" 
                        id="nip" 
                ...
                <div class="md:col-span-2 space-y-2">
                    <div class="flex items-center justify-between pl-1">
                        <label for="password" class="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Update Kata Sandi</label>
                        <span class="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">*Biarkan kosong jika tidak diubah</span>
                    </div>
                    <input 
                        type="password" 
                        name="password" 
            ...
            <div class="pt-6">
                <button type="submit" class="group relative w-full flex justify-center py-5 px-4 border border-transparent rounded-2xl text-sm font-black text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-all duration-300 overflow-hidden shadow-lg shadow-emerald-500/20">
                    <span class="absolute inset-0 w-full h-full bg-gradient-to-r from-emerald-400/20 to-rose-400/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out"></span>
