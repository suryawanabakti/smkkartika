        ...
        <div class="p-6 bg-white rounded-xl border border-gray-100 shadow-sm">
            <form action="{{ route('admin.teachers.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    @endif
                    <a href="{{ route('admin.teachers.pdf', request()->all()) }}" target="_blank"
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 sm:px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">NIP
                            </th>
                            <th
                                class="px-4 sm:px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">
                                Email</th>
                        ...
                        @forelse($teachers as $teacher)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 sm:px-6 py-4">
                                    ...
                                    <div class="md:hidden text-[10px] text-gray-500 truncate max-w-[150px]">
                                        {{ $teacher->user->email }}</div>
                                </td>
                                <td class="px-4 sm:px-6 py-4 text-gray-600">
                                    <span
                                        class="font-mono text-[10px] sm:text-xs bg-gray-100 px-2 py-1 rounded">{{ $teacher->nip }}</span>
                                </td>
                                <td class="px-4 sm:px-6 py-4 text-gray-500 hidden md:table-cell">{{ $teacher->user->email }}
                                </td>
