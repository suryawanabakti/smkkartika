...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-800">Edit Jadwal Mengajar</h1>
            <p class="text-gray-600">Perbarui informasi jadwal mengajar guru.</p>
        </div>

        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <form action="{{ route('admin.teacher-schedules.update', $teacherSchedule) }}" method="POST" class="p-6 space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <select name="day" id="day" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none @error('day') border-red-500 @enderror">
                            @foreach($days as $day)
                                <option value="{{ $day }}" {{ (old('day') ?? $teacherSchedule->day) == $day ? 'selected' : '' }}>{{ $day }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="class_id" id="class_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none @error('class_id') border-red-500 @enderror">
                            ...
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" {{ (old('class_id') ?? $teacherSchedule->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="subject" id="subject" value="{{ old('subject') ?? $teacherSchedule->subject }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none @error('subject') border-red-500 @enderror" placeholder="Contoh: Matematika">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="number" name="period_start" id="period_start" value="{{ old('period_start') ?? $teacherSchedule->period_start }}" min="1" max="11" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none @error('period_start') border-red-500 @enderror">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="number" name="period_end" id="period_end" value="{{ old('period_end') ?? $teacherSchedule->period_end }}" min="1" max="11" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none @error('period_end') border-red-500 @enderror">
                        @error('period_end') <p class="text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="pt-4 flex gap-3">
                    <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium">Perbarui Jadwal</button>
