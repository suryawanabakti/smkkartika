        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button @click="editMode = !editMode"
                    ...
                    <svg class="w-4 h-4" :class="editMode ? 'text-amber-600' : 'text-gray-400'" fill="none"
                ...
                <div class="px-4 py-3 bg-white rounded-xl border border-gray-100 shadow-sm">
                    <form action="{{ route('admin.attendance.personnel.recap') }}" method="GET"
                        class="flex flex-wrap items-center gap-3">
                        <div class="flex items-center gap-2">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Bulan</label>
                            <select name="month"
                                class="px-2 py-1.5 border border-gray-100 rounded-lg text-xs focus:ring-2 focus:ring-indigo-500 outline-none bg-gray-50 font-bold">
                        ...
                        <button type="submit"
                            class="px-4 py-1.5 bg-gray-800 text-white rounded-lg text-xs font-bold hover:bg-gray-900 transition-colors">
                            Filter
                        </button>
                        <a href="{{ route('admin.attendance.personnel.recap.pdf', request()->all()) }}" target="_blank"
                            class="px-4 py-1.5 bg-red-600 text-white rounded-lg text-xs font-bold hover:bg-red-700 transition-colors flex items-center gap-2">
                        ...
                        <button type="button" @click="showEmailModal = true"
                            class="px-4 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition-colors flex items-center gap-2">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <table class="w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-gray-100">
                            <th rowspan="2"
                                class="px-6 py-4 text-left font-bold text-slate-600 uppercase tracking-tight sticky left-0 bg-slate-50 z-20 border-r border-gray-100 min-w-[220px]">
                                NAMA / NIP
                            </th>
                            @for ($i = 1; $i <= $daysInMonth; $i++)
                                ...
                                @endphp
                                <th
                                    class="px-1 py-2 text-center font-bold border-r border-gray-50 {{ $isSunday ? 'bg-red-50 text-red-500' : 'text-slate-500' }}">
                                    {{ sprintf('%02d', $i) }}
                                </th>
                            @endfor
                        </tr>
                        <tr class="bg-slate-50/50 border-b border-gray-100">
                            @for ($i = 1; $i <= $daysInMonth; $i++)
                                ...
                                @endphp
                                <th
                                    class="px-1 py-1 text-center font-semibold border-r border-gray-50 {{ $isSunday ? 'bg-red-50 text-red-400' : 'text-slate-400 font-mono text-[9px]' }}">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($personnel as $person)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td
                                    class="px-6 py-4 sticky left-0 bg-white group-hover:bg-slate-50 z-10 border-r border-gray-100 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.05)]">
                                    <div class="font-bold text-slate-800 truncate">{{ $person->name }}</div>
                                    <div class="text-[10px] text-slate-400 font-mono mt-0.5">
                                        {{ $person->teacher->nip ?? 'A000000000001' }}</div>
                                </td>
                                @for ($i = 1; $i <= $daysInMonth; $i++)
                                    @php
                                        $date = $startDate->copy()->day($i);
                                        $isSunday = $date->isSunday();
                                        $attendance = isset($attendanceData[$person->id][$i])
                                            ? $attendanceData[$person->id][$i]->first(function ($a) use ($date) {
                                        ...
                                        if ($isSunday) {
                                            ...
                                            switch ($attendance->status) {
                                                case 'present':
                                                    $statusChar = 'H';
                                                    $cellClass = 'text-green-600 font-extrabold';
                                                    break;
                                                case 'sick':
                                                    $statusChar = 'S';
                                                    $cellClass = 'bg-yellow-50 text-yellow-600 font-extrabold';
                                                    break;
                                                case 'permission':
                                                    $statusChar = 'I';
                                                    $cellClass = 'bg-blue-50 text-blue-600 font-extrabold';
                                                    break;
                                                case 'absent':
                                                    $statusChar = 'A';
                                                    $cellClass = 'bg-red-50 text-red-600 font-extrabold animate-pulse';
                                                    break;
                                            }
                                        }
                                    @endphp
                                    <td @if (!$isSunday) @click="if(editMode && '{{ $status }}' !== 'present') openEdit('{{ $person->id }}', '{{ $person->name }}', '{{ $date->toDateString() }}', '{{ $status }}', '{{ addslashes($description) }}')" @endif
                                        ...
                                        :class="editMode && !{{ $isSunday ? 'true' : 'false' }} && '{{ $status }}'
                                        !== 'present' ?
                                            'hover:bg-indigo-50 hover:scale-110 cursor-pointer shadow-inner' : (
                                                editMode && '{{ $status }}'
                                                === 'present' ? 'opacity-50 cursor-not-allowed' : '')">
                                        <div class="relative group/cell">
                                            {{ $statusChar }}
                                            @if ($description)
                                                <div
                                                    class="absolute -top-1 -right-1 w-1.5 h-1.5 bg-indigo-400 rounded-full">
                                                </div>
                                                <div
                                                    class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-800 text-white text-[10px] rounded shadow-lg opacity-0 group-hover/cell:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                                                    {{ $description }}
                                                </div>
                                            @endif
                                        </div>
        ...
        <div class="p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-emerald-50/50 border border-emerald-100/50">
                    <div
                        class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-emerald-600 border border-emerald-100">
                        H</div>
                ...
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-amber-50/50 border border-amber-100/50">
                    <div
                        class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-amber-600 border border-amber-100">
                        S</div>
                    <div class="flex flex-col">
                ...
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-blue-50/50 border border-blue-100/50">
                    <div
                        class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-blue-600 border border-blue-100">
                        I</div>
                    <div class="flex flex-col">
                ...
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-rose-50/50 border border-rose-100/50">
                    <div
                        class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-rose-600 border border-rose-100">
                        A</div>
                    <div class="flex flex-col">
                ...
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/50 border border-slate-100/50">
                    <div
                        class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-slate-400 border border-slate-100">
                        L</div>
                    <div class="flex flex-col">
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
                    ...
                    class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" @click="showModal = false"
                        ...
                        <div class="p-8">
                            ...
                            <div class="flex items-start justify-between mb-8">
                                <div class="space-y-1">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-100 uppercase tracking-tighter"
                                            x-text="selectedDate"></span>
                                        <span class="text-slate-300 text-xs">•</span>
                                        <p class="text-xs font-medium text-slate-500 truncate max-w-[150px]"
                                            x-text="selectedUser"></p>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Pilih
                                        Status Kehadiran</label>
                                    <div class="grid grid-cols-2 gap-3">
                                        <!-- Status: Present -->
                                        <label class="relative cursor-pointer group">
                                            <input type="radio" name="status" value="present"
                                                x-model="selectedStatus" class="peer sr-only">
                                            <div
                                                class="p-3.5 rounded-2xl border-2 border-slate-50 bg-slate-50/50 transition-all flex flex-col gap-2 items-center text-center
                                                ...
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-emerald-500 peer-checked:scale-110 transition-transform">
                                        ...
                                        <!-- Status: Sick -->
                                        <label class="relative cursor-pointer group">
                                            <input type="radio" name="status" value="sick" x-model="selectedStatus"
                                                class="peer sr-only">
                                            <div
                                                class="p-3.5 rounded-2xl border-2 border-slate-50 bg-slate-50/50 transition-all flex flex-col gap-2 items-center text-center
                                                    ...
                                                    group-hover:bg-white group-hover:border-slate-200">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-amber-500 peer-checked:scale-110 transition-transform">
                                                    <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                        ...
                                        <label class="relative cursor-pointer group">
                                            ...
                                            <div
                                                class="p-3.5 rounded-2xl border-2 border-slate-50 bg-slate-50/50 transition-all flex flex-col gap-2 items-center text-center
                                                    ...
                                                    group-hover:bg-white group-hover:border-slate-200">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-blue-500 peer-checked:scale-110 transition-transform">
                                                    <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                        ...
                                        <!-- Status: Absent -->
                                        <label class="relative cursor-pointer group">
                                            <input type="radio" name="status" value="absent" x-model="selectedStatus"
                                                class="peer sr-only">
                                            <div
                                                class="p-3.5 rounded-2xl border-2 border-slate-50 bg-slate-50/50 transition-all flex flex-col gap-2 items-center text-center
                                                    ...
                                                    group-hover:bg-white group-hover:border-slate-200">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-rose-500 peer-checked:scale-110 transition-transform">
                                                    <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Keterangan
                            ...
                            <div class="mt-10 flex flex-col gap-3">
                                <button type="submit"
                                    class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:translate-y-[-2px] active:translate-y-[0px] transition-all text-sm">
                                    Simpan Perubahan
                                </button>
                                <button type="button" @click="showModal = false"
        ...
        <div x-show="showEmailModal" x-cloak class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showEmailModal" x-transition:enter="ease-out duration-300"
                    ...
                    @click="showEmailModal = false" aria-hidden="true"></div>
                    ...
                    <form action="{{ route('admin.attendance.personnel.recap.send_email') }}" method="POST">
                        ...
                        <div class="p-8">
                            <div class="flex items-start justify-between mb-6">
                                <div class="space-y-1">
                                    <h3 class="text-xl font-bold text-slate-800" id="modal-title">Kirim Rekap via Email
                                    </h3>
                                    <p class="text-xs font-medium text-slate-500">Kirim rekap kehadiran (Excel) ke email
                                        tujuan.</p>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Email
                                        Tujuan</label>
